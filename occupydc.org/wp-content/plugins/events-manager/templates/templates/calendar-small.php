...
<table class="em-calendar">
	<thead>
		<tr>
			<th><a class="em-calnav em-calnav-prev" href="<?php echo $calendar['links']['previous_url']; ?>" rel="nofollow">&lt;&lt;</a></th>
			<th class="month_name" colspan="5"><?php echo ucfirst(date_i18n('M Y', $calendar['month_start'])); ?></th>
			<th><a class="em-calnav em-calnav-next" href="<?php echo $calendar['links']['next_url']; ?>" rel="nofollow">&gt;&gt;</a></th>
		</tr>
	</thead>
	<tbody>
		<tr class="days-names">
		...
		<tr>
			...
			foreach($calendar['cells'] as $date => $cell_data ){
				...
				<td class="<?php echo $class; ?>">
					<?php if( !empty($cell_data['events']) && count($cell_data['events']) > 0 ): ?><script>

$(function ()

{ $("#<?php echo date('j',$cell_data['date']); ?>").popover();

});

</script>
					<a href="<?php echo esc_url($cell_data['link']); ?>"  id="<?php echo date('j',$cell_data['date']); ?>" data-content="<?php echo esc_attr($cell_data['link_title']); ?>" title="<?php echo esc_attr($cell_data['link_title']); ?>"><?php echo date('j',$cell_data['date']); ?></a>
					<?php else:?>
