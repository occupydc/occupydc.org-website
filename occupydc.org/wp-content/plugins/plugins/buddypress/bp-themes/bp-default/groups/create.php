...
<?php get_header( 'buddypress' ); ?>
	...
	<div id="content">
		...
		<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
			<h3><?php _e( 'Create a Committee', 'buddypress' ); ?> &nbsp;<a class="button" href="<?php echo trailingslashit( bp_get_root_domain() . '/' . bp_get_groups_root_slug() ); ?>"><?php _e( 'Committees Directory', 'buddypress' ); ?></a></h3>
			...
			<div class="item-body" id="group-create-body">
				...
				<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
					...
					<label for="group-name"><?php _e( 'Committee Name (required)', 'buddypress' ); ?></label>
					<input type="text" name="group-name" id="group-name" aria-required="true" value="<?php bp_new_group_name(); ?>" />

					<label for="group-desc"><?php _e( 'Committee Description (required)', 'buddypress' ) ?></label>
				...
				<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
					...
					<?php if ( bp_is_active( 'forums' ) ) : ?>
						...
						<?php else : ?>
							<?php if ( is_super_admin() ) : ?>
								...
								<div class="checkbox">
									<label><input type="checkbox" disabled="disabled" name="disabled" id="disabled" value="0" /> <?php printf( __( '<strong>Attention Site Admin:</strong>Committee forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'buddypress' ), bp_get_root_domain() . '/wp-admin/admin.php?page=bb-forums-setup' ); ?></label>
								</div>
					...
					<div class="radio">
						<label><input type="radio" name="group-status" value="public"<?php if ( 'public' == bp_get_new_group_status() || !bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							<strong><?php _e( 'This is a public Committee', 'buddypress' ); ?></strong>
							<ul>
								<li><?php _e( 'Any site member can join this Committee.', 'buddypress' ); ?></li>
								<li><?php _e( 'This Committee  will be listed in the Committee(s) directory and in search results.', 'buddypress' ); ?></li>
								<li><?php _e( 'Committee content and activity will be visible to any site member.', 'buddypress' ); ?></li>
							</ul>
						</label>

						<label><input type="radio" name="group-status" value="private"<?php if ( 'private' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							<strong><?php _e( 'This is a private Committee', 'buddypress' ); ?></strong>
							<ul>
								<li><?php _e( 'Only users who request membership and are accepted can join the Committee.', 'buddypress' ); ?></li>
								<li><?php _e( 'This Committee will be listed in the groups directory and in search results.', 'buddypress' ); ?></li>
								<li><?php _e( 'Committee content and activity will only be visible to members of the Committee.', 'buddypress' ); ?></li>
							</ul>
						</label>

						<label><input type="radio" name="group-status" value="hidden"<?php if ( 'hidden' == bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> />
							<strong><?php _e('This is a hidden Committee', 'buddypress'); ?></strong>
							<ul>
								<li><?php _e( 'Only users who are invited can join the Committee.', 'buddypress' ); ?></li>
								<li><?php _e( 'This group will not be listed in the Committee(s) directory or search results.', 'buddypress' ); ?></li>
								<li><?php _e( 'Committee content and activity will only be visible to members of the Committee.', 'buddypress' ); ?></li>
							</ul>
						</label>
					</div>
					...
					<p><?php _e( 'Which members of this Committee are allowed to invite others?', 'buddypress' ) ?></p>

					<div class="radio">
						<label>
							<input type="radio" name="group-invite-status" value="members"<?php bp_group_show_invite_status_setting( 'members' ) ?> />
							<strong><?php _e( 'All Committee members', 'buddypress' ) ?></strong>
						</label>

						<label>
							<input type="radio" name="group-invite-status" value="mods"<?php bp_group_show_invite_status_setting( 'mods' ) ?> />
							<strong><?php _e( 'Committee admins and mods only', 'buddypress' ) ?></strong>
						</label>

						<label>
							<input type="radio" name="group-invite-status" value="admins"<?php bp_group_show_invite_status_setting( 'admins' ) ?> />
							<strong><?php _e( 'Committee admins only', 'buddypress' ) ?></strong>
						</label>
					</div>
				...
				<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
					...
					<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
						...
						<div class="main-column">
							<p><?php _e( "Upload an image to use as an avatar for this Committee. The image will be shown on the main Committee page, and in search results.", 'buddypress' ); ?></p>
				...
				<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
					...
					<?php else : ?>
						...
						<div id="message" class="info">
							<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your Committee. You can send invites any time in the future by selecting the "Send Invites" option when viewing your new Committee.', 'buddypress' ); ?></p>
						</div>
				...
				<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
					...
					<div class="submit" id="previous-next">
						...
						<?php if ( bp_is_first_group_creation_step() ) : ?>
							...
							<input type="submit" value="<?php _e( 'Create Committee and Continue', 'buddypress' ); ?>" id="group-creation-create" name="save" />
